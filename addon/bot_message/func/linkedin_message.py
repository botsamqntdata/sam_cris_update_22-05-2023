...
def send_linkedin(browser, url, email, subject, message, file, min_delay):
    ...
    try:
        premium= browser.find_element_by_xpath('//a[contains(@class,app-aware-link) and contains(@class, "artdeco-button") and contains(@class,"artdeco-button--premium")]')
        href = premium.get_property('href')
    except:
        href=''
        pass

    if 'https://www.linkedin.com/premium/products/' not in href:
    
        try:
            elementID = browser.find_element_by_xpath(
                '//input[@accept="image/*,.ai,.psd,.pdf,.doc,.docx,.csv,.zip,.rar,.ppt,.pptx,.pps,.ppsx,.odt,.rtf,.xls,.xlsx,.txt,.pub,.html,.7z,.eml,.mov,.mp4"]'
        ...
        try:
            browser.find_element_by_xpath(
                '//input[@name="subject"]').send_keys(subject)
        except:
            pass

        browser.find_element_by_xpath(
            '//div[@role="textbox" and @aria-multiline="true"]').click()
        ...
        send_button = browser.find_element_by_class_name(
        ...
        if clickable:
            send_button.click()
    elif 'https://www.linkedin.com/premium/products/'  in href:
        # exitview= browser.find_element_by_xpath('//div[contains(@class,"artdeco-modal-overlay--layer-default ")]').click()
        # connect=browser.find_element_by_xpath('//div[contains(@class),"artdeco-button" and contains(@class),"artdeco-button--2" and contains(@class),"artdeco-button--primary" and contains(@class),"artdeco-button"]')
